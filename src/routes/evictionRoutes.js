...
router.get('/lru/demo', (req, res) => {
  ...
  // Isi cache dengan 5 artikel (kapasitas = 5)
  ['fiksi','nonfiksi','sains','sejarah','teknologi'].forEach(k => { eviction.lru.set(k, `koleksi-${k}`); log.push(`SET ${k}`); });
  log.push('--- Cache penuh (kapasitas: 5) ---');

  // Akses nonfiksi dan sejarah (memindahkannya ke posisi MRU)
  eviction.lru.get('nonfiksi'); log.push('GET nonfiksi  → dipindah ke posisi MRU');
  eviction.lru.get('sejarah'); log.push('GET sejarah  → dipindah ke posisi MRU');
  log.push(`Inspeksi sebelum eviction: ${JSON.stringify(eviction.lru.inspect().map(i => `${i.key}(rank:${i.rank})`))}`);

  // Insert sastra → seharusnya mengeluarkan berita:1 (paling lama tidak diakses)
  eviction.lru.set('sastra', 'koleksi-sastra'); log.push('SET sastra  → fiksi di-evict (paling lama tidak diakses)');
...
router.get('/fifo/demo', (req, res) => {
  ...
  ['2019','2020','2021','2022','2023'].forEach(k => { eviction.fifo.set(k, `terbitan-${k}`); log.push(`SET ${k}`); });
  log.push('--- Cache penuh ---');

  // Urutan akses TIDAK mempengaruhi FIFO
  eviction.fifo.get('mei'); eviction.fifo.get('mei'); eviction.fifo.get('mei');
  log.push('GET mei x3 — frekuensi/recency tidak berpengaruh di FIFO');

  eviction.fifo.set('2024', 'terbitan-2024');
  log.push('SET jun \u2192 2019 di-evict (masuk pertama, dikeluarkan pertama)');
...
router.get('/ttl/demo', async (req, res) => {
  ...
  eviction.ttl.set('arsip-lama',   'berita arsip',     0);  log.push('SET arsip-lama    TTL=0 (tidak kedaluwarsa)');
  eviction.ttl.set('breaking-now', 'berita terkini',   2);  log.push('SET breaking-now  TTL=2s');
  eviction.ttl.set('berita-hari',  'berita hari ini', 10);  log.push('SET berita-hari   TTL=10s');
  eviction.ttl.set('berita-minggu','berita minggu ini',60); log.push('SET berita-minggu TTL=60s');
  ...
  res.json({
    ...
    hint:         'Coba GET /api/eviction/ttl/get/breaking-now setelah 2 detik untuk melihat hasilnya null (kedaluwarsa)',
  });
});
